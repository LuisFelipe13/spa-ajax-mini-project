...
class SourcesList {
  constructor () {
    this.sources = []
    this.sourceContainer = document.getElementById('sources')
  ...
  fetchSources() {
    this.adapter.getSources()
      .then(this.getJson)
      .then(this.createSources.bind(this))
  ...
  createSources(resp) {
    this.sources = resp.sources.map(source => new Source())
  }

  render() {
    this.sourceContainer.innerHTML = this.renderSources()
  }
